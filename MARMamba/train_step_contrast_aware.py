...
import cv2
from utils.aapm_dataset import test_image, AAPMTrainDataset as MARTrainDataset
...
# --- Contrast-aware loss hyper-parameters (bo sung) --- #
parser.add_argument('-gamma', help='Weight for contrast-aware loss', default=0.1, type=float)
parser.add_argument('-ca_window', help='Window size for local std in contrast-aware loss', default=5, type=int)
...
print(
    'learning_rate: {}\ncrop_size: {}\ntrain_batch_size: {}\nval_batch_size: {}\ngamma: {}\nca_window: {}\n'.format(
        learning_rate, crop_size, train_batch_size, val_batch_size, gamma, ca_window))
...
# --- Contrast-aware loss (bo sung) --- #
def local_std(gt, window=5):
    """Tinh do lech chuan cuc bo trong cua so window x window quanh moi pixel cua ground truth."""
    pad = window // 2
...
def contrast_aware_loss(pred, gt, window=5, eps=0.00125):
    """
    Chuan hoa sai so theo do tuong phan cuc bo (Contrast-aware Loss).
    eps=0.00125 tuong ung ~5 HU trong thang HU_MIN=-1000, HU_MAX=3000
    (5 / 4000 = 0.00125) -- khop voi nguong chenh lech mo mem trong de xuat.
    """
...
while True:
    for batch_id, train_data in enumerate(lbl_train_data_loader):
        ...
        loss = (0.8 * hub_loss(pred_image, gt)
                + 0.2 * lpips_loss(pred_image, gt).mean()
                + gamma * contrast_aware_loss(pred_image, gt, window=ca_window))
