...
def run_eval(test_data_dir, checkpoint, output_dir, device="cuda"):
    ...
    overall_results = {"non_metal": [], "metal_included": []}
    ...
    # Kết quả theo đúng số lượng kim loại.
    # AAPM CT-MAR hiện tại có các nhóm 1..5 metal.
    metal_count_results = {
        n: {"non_metal": [], "metal_included": []}
        for n in range(1, 6)
    }
    ...
    with torch.no_grad():
        for idx, s in enumerate(samples):
            ...
            for exclude_metal, key in [(True, "non_metal"), (False, "metal_included")]:
                ...
                if n_materials in metal_count_results:
                    metal_count_results[n_materials][key].append(entry)
    ...
    summary = {
        "overall": {
            k: summarize(overall_results[k])
            for k in ["non_metal", "metal_included"]
        },
        "by_size": {},
        "by_multiplicity": {},
        "by_metal_count": {},
    ...
    for group in ["Large", "Medium", "Small", "Tiny"]:
        summary["by_size"][group] = {
            k: summarize(results[group][k])
            for k in ["non_metal", "metal_included"]
        }

    for group in ["single", "multi"]:
        summary["by_multiplicity"][group] = {
            k: summarize(multi_results[group][k])
            for k in ["non_metal", "metal_included"]
        }

    for n_materials in range(1, 6):
        summary["by_metal_count"][str(n_materials)] = {
            k: summarize(metal_count_results[n_materials][k])
            for k in ["non_metal", "metal_included"]
        }
    ...
    print("\n=== BẢNG PHỤ: theo số lượng kim loại (1/2/3/4/5) ===")
    for n_materials in range(1, 6):
        print(f"\n-- {n_materials} metal --")
        for key, label in [
            ...
            r = summary["by_metal_count"][str(n_materials)][key]
            if r is None:
                print(f"  {label}: (không có sample)")
                continue
            print(
