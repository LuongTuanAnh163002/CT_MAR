"""
AAPM CT-MAR evaluation for Metal-Geometry Guided Dynamic FMB.

The evaluation protocol is intentionally kept identical to the supplied
`test_aapm.py`:
  * Large/Medium/Small/Tiny = quartiles of true metal-mask pixel area.
  * Single/Multi = n_materials from metalinfo JSON.
  * Non-metal evaluation = zero metal pixels in BOTH prediction and GT.
  * PSNR/SSIM/RMSE/LPIPS implementation is unchanged.

Only the model import/loading and forward pass are changed to provide the
binary metal mask required by Dynamic FMB. Guidance weights are saved only
...
def run_eval(test_data_dir, checkpoint, output_dir, device="cuda", guidance_temperature=1.0):
    ...
    net = load_model(checkpoint, device, guidance_temperature=guidance_temperature)
    ...
    with torch.no_grad():
        for idx, s in enumerate(samples):
            ...
            if mask is None:
                raise RuntimeError(
                    f"Dynamic FMB cần metal mask nhưng sample {s['id']} không có mask."
                )
            ...
            # Dynamic FMB forward: exactly the same CT input as baseline,
            # plus the binary metal mask required by the guidance network.
            output, guidance = net(
                input_t,
                mask_t,
            ...
            # Diagnostic only; does not affect any evaluation metric/grouping.
            if guidance is not None:
                w = guidance.squeeze(0).detach().cpu().numpy()
    ...
    summary = {"by_size": {}, "by_multiplicity": {}, "guidance": {}}
    ...
    # Extra Dynamic FMB routing diagnostics.
    if guidance_records:
        weights = np.array(
            [[r["w0"], r["w1"], r["w2"]] for r in guidance_records],
            dtype=np.float64,
        )
        summary["guidance"] = {
            "n": len(guidance_records),
            "w0": {"mean": float(weights[:, 0].mean()), "std": float(weights[:, 0].std()),
                   "min": float(weights[:, 0].min()), "max": float(weights[:, 0].max())},
            "w1": {"mean": float(weights[:, 1].mean()), "std": float(weights[:, 1].std()),
                   "min": float(weights[:, 1].min()), "max": float(weights[:, 1].max())},
            "w2": {"mean": float(weights[:, 2].mean()), "std": float(weights[:, 2].std()),
                   "min": float(weights[:, 2].min()), "max": float(weights[:, 2].max())},
        }

        import csv
        guidance_csv = os.path.join(output_dir, "guidance_weights.csv")
        with open(guidance_csv, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f,
                fieldnames=["id", "group", "mask_size", "n_materials", "w0", "w1", "w2"],
            )
            writer.writeheader()
            writer.writerows(guidance_records)
    ...
    if summary.get("guidance"):
        ...
        for name in ["w0", "w1", "w2"]:
            r = summary["guidance"][name]
            print(
                f"{name}: mean={r['mean']:.6f}, std={r['std']:.6f}, "
                f"min={r['min']:.6f}, max={r['max']:.6f}"
            )
        print(f"Guidance CSV: {os.path.join(output_dir, 'guidance_weights.csv')}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Softmax temperature of MetalGeometryGuidance; keep identical to training.",
    )
